...
<body class="fix-header hg-admin-portal">
    ...
    <div id="main-wrapper">        <?php $hg_admin_page = 'menu'; include __DIR__ . '/includes/admin_shell.php'; ?>
													...
													if(!mysqli_num_rows($query) > 0 )
														...
														{				
																	while($rows=mysqli_fetch_array($query))
																		{
																				$mql="select * from hawkerstalls where rs_id='".$rows['rs_id']."'";
																				...
																				$fetch=mysqli_fetch_array($newquery);
																					...
																					echo '<tr><td>'.$fetch['title'].'</td>
																								...
																								<td>'.$rows['slogan'].'</td>
																								<td>$'.$rows['price'].'</td>
																								<td>'.(intval($rows['is_sold_out']) === 1 ? '<span class="badge badge-danger">Sold Out</span>' : '<span class="badge badge-success">Available</span>').'
                                                                                                <br><a href="toggle_soldout.php?d_id='.$rows['d_id'].'" class="btn btn-xs btn-warning mt-1">Toggle</a></td>
